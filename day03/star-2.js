class Board {
	...
	execute (input) {
				...
				switch (match[1]) {
					case 'R':
						for (let i = 1; i <= parseInt(match[2], 10); i++) {
							x += 1;
							steps[wireIndex].counter++;
							const key = `${x},${y}`;
							board[key] = board[key] === undefined ? wireIndex : (board[key] === 5 ? 5 : (board[key] !== wireIndex ? 5 : wireIndex));
							if(steps[wireIndex].values[key] === undefined) {
								steps[wireIndex].values[key] = steps[wireIndex].counter;
							}
						}
						break;
					case 'L':
		...
		for (let key of Object.keys(board)) {
			if (board[key] === 5) {
				minSteps = Math.min(minSteps, steps[0].values[key] + steps[1].values[key]);
			}
		}
