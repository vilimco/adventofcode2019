	...
	execute (input) {
		...
		for (let wire of input) {
			...
			for (let direction of wire) {
				const match = /(R|L|U|D)(\d+)/.exec(direction);

				switch (match[1]) {
					case 'R':
						for (let i = 1; i <= parseInt(match[2], 10); i++) {
							x += 1;
							const key = `${x},${y}`;
							board[key] = board[key] === undefined ? wireIndex : (board[key] === 5 ? 5 : (board[key] !== wireIndex ? 5 : wireIndex));
						}
						break;
					case 'L':
						for (let i = 1; i <= parseInt(match[2], 10); i++) {
		...
		for (let key of Object.keys(board)) {
			if (board[key] === 5) {
				const [x, y] = key.split(',').map((num) => parseInt(num, 10));
				max = Math.min(max, this.calculateDistance(x0, y0, x, y));
			}
		}
		console.log(max);
	}

	calculateDistance (x0, y0, x, y) {
		// cover this here just so we don't have to check every time this is called.
		// we will call this an system exception
		if (x0 === x && y0 === y) {
			return Infinity;
		}
